...
#generate lifted file for all dosages, split by chr:

declare -a drugs=("apix" "clop" "riva" "noac")
for drug in ${drugs[@]}
  do
  for chr in {1..22}
    do awk '{print $1 FS $3 FS $3+1 FS $2}' /home/ashley/account/dosages/blacks/${drug}/pred_dosages/${drug}_chr${chr}.dosage.txt.gz > /home/ashley/account/liftover/lifted_dosages_b37/${drug}/prelifted_files/${drug}_chr${chr}_prelift.txt
