...
for drug in ${drugs[@]}
  ...
  for model in ${models[@]}
    do
    /usr/local/bin/gemma -g /home/ashley/account/pwas_results/blacks/${drug}/assoc_input_gemma/blacks_${drug}_PCAIR_PAV_filtered_${model}_baseline_rho0.1_zpval0.05_gemma.txt \
    -notsnp \
